...
<div class="superwisor_inbox_page main_section bg-white m-4 pb-5">    
    ...
    <div class="d-flex justify-content-between align-items-center px-5 pt-3">
        <h3 class="pt-4 text-bold text-left text-uppercase">Pending MRF (Completed Initial Inspection)</h3>
    ...
    <div class="container-fluid px-5 mx-3">
        <div class="row mt-3 table-responsive">
            @if(count($pending_project_with_inspection) > 0)
            <table class="table table-hover table-bordered w-100 text-center" id="pending_project_inspected_table">
                <thead>
                    <tr>
                ...
                <tbody>
                    @foreach($pending_project_with_inspection as $val)
                    <?php
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="project_check_status">
                                <a class="btn btn-primary primary_bg_color text-white p-0"
                                    href="{{ route('MRFExcelDownloadInspected', [
                                                                                    'article_number' => $val->full_article_number, 
                                                                                    'description' => $val->description, 
                                                                                    'qty' => $val->qty,
                                                                                    'project_no' => $val->project->project_no,
                                                                                    'all_item_from_stock'=> false
                                                                                ]) }}">
                                    MRF To Warehouse
                                </a>
                            </span>
                        ...
                        <td>
                            <span class="project_check_status">
                                <button type="button" class="btn btn-primary primary_bg_color text-white py-1 px-2 send-email-inspected-btn"
                                    data-product-id="{{ $val->id }}" data-project-id="{{ $val->project_id }}">
                                    Send Email
                                </button>
                            </span>
                        </td>
            ...
            <div class="alert alert-info w-100">
                No Pending MRF with completed initial inspection found.
            </div>
            @endif
        </div>
    </div>  
    <div class="d-flex justify-content-between align-items-center px-5 pt-3">
        <h3 class="pt-4 text-bold text-left text-uppercase">Pending MRF (All Items Are From Stock)</h3>
        <a class="mt-4 mb-2 btn btn-primary btn-sm py-2 px-3" onclick="location.reload();">
            <i class="fa fa-refresh text-light">&#xf021;</i>
        </a>
    </div>
    ...
    <div class="container-fluid px-5 mx-3">
        <div class="row mt-3 table-responsive">
            @if($all_items_are_from_stock_pending_mrf->isNotEmpty())
            <table class="table table-hover table-bordered w-100 text-center" id="pending_project_all_items_are_from_stock">
                <thead>
                    <tr>
                        <th scope="col" class="project_table_heading">Request Date</th>
                        <th scope="col" class="project_table_heading">Deadline</th>
                ...
                <tbody>
                    @foreach($all_items_are_from_stock_pending_mrf as $val)
                    <?php
                    $orderDate = $val->processed_at ? \Carbon\Carbon::parse($val->processed_at) : $val->created_at;
                    $currentDateTime = \Carbon\Carbon::now('Asia/Kolkata');
                    $deadlineClass = $val->deadline && $currentDateTime->gte($val->deadline) ? 'text-danger' : 'text-success';
                    ?>
                    <tr>
                        ...
                        <td>
                            <span class="project_check_status">
                                <button type="button" class="btn btn-primary primary_bg_color text-white py-1 px-2 send-email-from-stock-btn"
                                    data-product-id="{{ $val->id }}" data-project-id="{{ $val->project_id }}">
                                    Send Email
                                </button>
                            </span>
    ...
    <div class="d-flex justify-content-between align-items-center px-5 pt-3">
        <h3 class="pt-4 text-bold text-left text-uppercase">Pending MRF (Awaiting Initial Inspection)</h3>
        <a class="mt-4 mb-2 btn btn-primary btn-sm py-2 px-3" onclick="location.reload();">
            <i class="fa fa-refresh text-light">&#xf021;</i>
        </a>
    </div>
    ...
    <div class="container-fluid px-5 mx-3">
        <div class="row mt-3 table-responsive">
            @if(count($pending_project_without_inspection) > 0)
            <table class="table table-hover table-bordered w-100 text-center" id="pending_project_not_inspected_table">
                <thead>
                    <tr>
                        <th scope="col" class="project_table_heading">Request Date</th>
                        <th scope="col" class="project_table_heading">Deadline</th>
                ...
                <tbody>
                    @foreach($pending_project_without_inspection as $val)
                    <?php
                    $orderDate = $val->ini_inspection_date ? \Carbon\Carbon::parse($val->ini_inspection_date) : $val->created_at;
                    $currentDateTime = \Carbon\Carbon::now('Asia/Kolkata');
                    $deadlineClass = $val->deadline && $currentDateTime->gte($val->deadline) ? 'text-danger' : 'text-success';
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="project_check_status">
                                <a class="btn btn-primary primary_bg_color text-white p-0"
                                    href="{{ route('MRFExcelDownloadNotInspected', ['article_number' => $val->full_article_number, 'description' => $val->description, 'qty' => $val->qty, 'project_no' => $val->project->project_no]) }}">
                                    Pending MRF
                                </a>
                            </span>
                        </td>
            ...
            @else
            <div class="alert alert-info w-100">
                No Pending MRF awaiting initial inspection found.
            </div>
            @endif
        </div>
    </div>
    ...
    <div class="d-flex justify-content-between align-items-center px-5 pt-3">
        <h3 class="pt-4 text-bold text-left text-uppercase">Pending Response from Warehouse Persons</h3>
        <a class="mt-4 mb-2 btn btn-primary btn-sm py-2 px-3" onclick="location.reload();">
            <i class="fa fa-refresh text-light">&#xf021;</i>
        </a>
    </div>
    ...
    <div class="container-fluid px-5 mx-3">
        <div class="row mt-3 table-responsive">
            @if(count($processing_mrf) > 0)
            <table class="table table-hover table-bordered w-100 text-center" id="processing_mrf_table">
                <thead>
                    <tr>
                        <th scope="col" class="project_table_heading">Request Date</th>
                        <th scope="col" class="project_table_heading">Deadline</th>
    ...
    <div class="d-flex justify-content-between align-items-center px-5 pt-3">
        <h3 class="pt-4 text-bold text-left text-uppercase">Ready MRF (Full Or Partial)</h3>
        <a class="mt-4 mb-2 btn btn-primary btn-sm py-2 px-3" onclick="location.reload();">
            <i class="fa fa-refresh text-light">&#xf021;</i>
        </a>
    </div>
    ...
    <div class="container-fluid px-5 mx-3">
        <div class="row mt-3 table-responsive">
            ...
            <table class="table table-hover table-bordered w-100 text-center" id="ready_mrf_table">
                <thead>
                    <tr>
                        <th scope="col" class="project_table_heading">Request Date</th>
                        <th scope="col" class="project_table_heading">Deadline</th>
...
<script>
    $(document).ready(function() {
        ...
        $('#pending_project_table').removeClass('dataTable');

        $('#processing_mrf_table').DataTable({
            paging: true,
            pageLength: 2,
            lengthMenu: [2, 5, 10, 25, 50, 100],
            searching: true,
        ...
        $('#processing_mrf_table').removeClass('dataTable');
        ...
        $('#ready_mrf_table').removeClass('dataTable');
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        // Upload MRF - Inspected Items
        $(document).on('click', '.upload-mrf-inspected-btn', function(e) {
            e.preventDefault();
            e.stopPropagation();
            const $input = $(this).closest('.project_check_status').find('.upload-mrf-inspected-input');
            if ($input.length) {
                $input.trigger('click');
            }
        });

        $(document).on('change', '.upload-mrf-inspected-input', function() {
            const file = this.files[0];
            const productId = $(this).data('product-id');
            const projectId = $(this).data('project-id');
            const csrfToken = "{{ csrf_token() }}";
            const $button = $(this).closest('.project_check_status').find('.upload-mrf-inspected-btn');
            const originalText = $button.text();
        ...
        // Upload MRF - Not Inspected Items
        $(document).on('click', '.upload-mrf-not-inspected-btn', function(e) {
            e.preventDefault();
            e.stopPropagation();
            const $input = $(this).closest('.project_check_status').find('.upload-mrf-not-inspected-input');
            if ($input.length) {
                $input.trigger('click');
            }
        });

        $(document).on('change', '.upload-mrf-not-inspected-input', function() {
            const file = this.files[0];
            const productId = $(this).data('product-id');
            const projectId = $(this).data('project-id');
            const csrfToken = "{{ csrf_token() }}";
            const $button = $(this).closest('.project_check_status').find('.upload-mrf-not-inspected-btn');
            const originalText = $button.text();
        ...
        // Send Email - Inspected Items ONLY
        $(document).on('click', '.send-email-inspected-btn', function(e) {
            e.preventDefault();
            e.stopPropagation();
            const productId = $(this).data('product-id');
            const projectId = $(this).data('project-id');
            ...
            if (confirm('Are you sure you want to send the MRF email to Warehouse Coordinators?')) {
                ...
                $.ajax({
                    url: "{{ route('SendMRFEmailInspected') }}",
                    type: 'POST',
                    data: {
                        _token: csrfToken,
                        product_id: productId,
         ...
         // Send Email - From Stock Items ONLY
        $(document).on('click', '.send-email-from-stock-btn', function(e) {
            e.preventDefault();
            e.stopPropagation();
            const productId = $(this).data('product-id');
            const projectId = $(this).data('project-id');
            ...
            if (confirm('Are you sure you want to send the MRF email to Warehouse Coordinators?')) {
                ...
                $.ajax({
                    url: "{{ route('SendMRFEmailFromStock') }}",
                    type: 'POST',
                    data: {
                        _token: csrfToken,
                        product_id: productId,
