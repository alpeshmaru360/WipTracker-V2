...
// public function getTimerState(Request $request)
    ...
    //                     $op['status'] = $op['status'] ?? null;
    //                     $op['total_time'] = $op['total_time'] ?? 0;
                        
    //                     // if ($op['session_start'] === null && $op['status'] === 'running') {
    //                     if($op['status'] === 'running' && !empty($op['session_start'])) {
    //                         $sessionStart = Carbon::parse($op['session_start']);
    //                         $liveTime = $sessionStart->diffInSeconds(now());

    //                         // ⚠️ Important: don’t add live time directly to total_time here,
    //                         // because on every refresh it keeps increasing.
    //                         $calculatedTotal = ($op['total_time'] ?? 0) + $liveTime;
    //                     } else {
    //                         $liveTime = $op['total_time'] ?? 0;
    //                         $calculatedTotal = $liveTime;
    //                     }
    ...
    //                         'process_status' => $process->timer_status,
    //                         'remaining_time' => $remainingTime,
    //                         'elapsed_time' => $liveTime,
    //                         'timer_started_at' => $process->timer_started_at,
    //                         'timer_current_time' => $process->timer_current_time,
    //                         'project_status' => $process->project_status,
    //                         'actual_time' => $this->convertSecondsToHMS($liveTime),
    //                         'operatorId' => $operatorId,
    //                         'uniqueId' => $uniqueId,
    //                     ];

    //                     // ✅ Update only this operator in DB
    //                     $op['total_time'] = $calculatedTotal;
    //                 }
    //             }
    //             // unset($op);
    //             //
    //             $originalData = $process->operators_time_tracking;
    //             $newData = json_encode($operators);

    //             if ($originalData !== $newData) {
    //                 $process->operators_time_tracking = $newData;
    //                 $process->save();
    //             }
    //             //
    //             // if ($updated) {
...
public function startTimer(Request $request)
{
    DB::transaction(function () use ($request) {
        ...
        // Update or create operator tracking record
        ...
        // Update process status only if not already running
    ...
    $uniqueId = $request->projectId . '-' . $request->productId . '-' . $request->orderQty .'-' . $request->operatorId;
   // broadcast(new ProductProcessStartedEvent($request->uniqueId, $request->operatorId));
    broadcast(new ProductProcessStartedEvent(
            $request->uniqueId,
            $request->projectId,
            $request->productId,
...
public function getTimerState(Request $request)
{
    $processes = ProjectProcessStdTime::where('projects_id', $request->projectId)
        ->where('product_id', $request->productId)
        ->where('order_qty', $request->orderQty)
        ->get(); // Get all processes, not just first one
    ...
    foreach ($processes as $key => $process) {
        ...
        foreach ($operatorTrackings as $tracking) {
            ...
            // This should match the data-unique-id in your HTML
    ...
    return response()->json([
        'timer_states' => $timerStates,
        'serverTime' => now()->timestamp * 1000 // Add server time for sync
    ]);
}
...
private function calculateLiveTime($operatorData, $processStatus)
{
    ...
    if (($operatorStatus === 'running') && !empty($operatorData['session_start'])) {
        try {
            ...
            \Log::error('Error calculating live time:', ['error' => $e->getMessage()]);
